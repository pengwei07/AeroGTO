...
class AttentionBlock(paddle.nn.Layer):
    def __init__(self, n_token=64, w_size=128, n_heads=4):
        ...
        self.attention2 = paddle.nn.MultiHeadAttention(
            embed_dim=w_size, num_heads=n_heads
        )
...
class Mixer(paddle.nn.Layer):
    ...
    def forward(self, V, edges, node_pos, s_enc):
        ...
        norm = paddle.sqrt(x=(distance**2).sum(-1, keepdims=True))
...
class TransformerWithFixedQuery(paddle.nn.Layer):
    def __init__(self, input_dim, hidden_dim, num_heads, output_dim):
        ...
        self.query = paddle.base.framework.EagerParamBase.from_tensor(
            tensor=paddle.randn(shape=[1, 1, input_dim])
        )
        self.self_attention = paddle.nn.MultiHeadAttention(
            embed_dim=input_dim, num_heads=num_heads
        )
        self.ffn = paddle.nn.Sequential(
            paddle.nn.Linear(in_features=input_dim, out_features=hidden_dim),
            paddle.nn.PReLU(),
            paddle.nn.Linear(in_features=hidden_dim, out_features=output_dim),
        )

    def forward(self, x):
        B, N, C = tuple(x.shape)
        query = self.query.expand(shape=[B, 1, C])
        attn_output, _ = self.self_attention(query, x, x)
        output = self.ffn(attn_output.squeeze(axis=1))
        return output
...
class time_stepping(paddle.nn.Layer):
    def __init__(
        ...
        self.Transformer = TransformerWithFixedQuery(
            state_embedding_dim, att_embedding_dim, n_head, state_embedding_dim
        )
...
class AeroGTO_cd(paddle.nn.Layer):
    def __init__(
        self,
        N_block=4,
        state_size=4,
