...
class GNOT(nn.Module):
    ...
    def forward(self, node_pos, areas, info):
        # def forward(self, inputs, device, infer_times):
        
        x = torch.cat([node_pos], dim=-1)

        mesh_info = [areas]
        # model calculate
        x = self.trunk_mlp(x) # num of graphs, num of points, 2
        z = MultipleTensors([self.branch_mlps[i](mesh_info[i]) for i in range(self.n_inputs)])
        for block in self.blocks:
            x = block(x, z, node_pos)
        f_t = self.out_mlp(x)
...
if __name__ == "__main__":
    ...
    params_1 = sum([np.prod(p.size()) for p in model_parameters])
    print(f"Number of parameters: {params_1/1e6:.3f}M")
    points = torch.rand(1,1000,3)
    areas = torch.rand(1,2000,1)
    y = model(points, areas)
    print(y.shape)
