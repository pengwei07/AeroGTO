...
def read_vtk(file_path):
    ...
    for i in range(num_cells):
        if cell_types.GetValue(i) == hexahedron_type:  # 检查是否为六边形
            cell = vtk.vtkIdList()
    ...
    # 获取速度数据
    velocity = vtk_to_numpy(data.GetPointData().GetArray("point_vectors")).astype(np.float32)
...
class Velocity_Dataset(Dataset):
    ...
    def get_singel(self, item):
        ...
        node_pos, edges, velocity = get_data(
            self.fn,
            self.dataloc[item]
            )
    ...
    def scale_pos(self, pos):
        pos_min = torch.tensor([-1.5696, -0.2591, -3.1480]).to(pos.device)
        pos_max = torch.tensor([1.5658, 2.0213, 4.1869]).to(pos.device)
