...
class GNOFNOGNOAhmed(GNOFNOGNO):
    ...
    def data_dict_to_input_test(self, data_dict, device_test):
        ...
        # x_out = (data_dict["df_query_points"].squeeze(0).permute(1, 2, 3, 0))  
        ...
        info_fields = data_dict['info']['velocity'] * torch.ones_like(df)
        ...
        if self.use_adain:
            vel = torch.tensor([data_dict['info']['velocity']]).view(-1, ).to(new_device)
            vel_embed = self.adain_pos_embed(vel)
            for norm in self.fno.fno_blocks.norm:
                norm.update_embeddding(vel_embed)
    ...
    def eval_dict(self, device, data_dict, loss_fn=None, decode_fn=None, **kwargs):
        ...
        # print(pred.shape)
        # print(data_dict["pressure"].shape, data_dict['dragWeight'][0].shape)
        # print(data_dict['dragWeight'].shape, data_dict['dragWeight'][0].shape,)
        
        truth = data_dict["pressure"][0].to(device)[::self.subsample_eval, ...].reshape(1, -1)
        ...
        if decode_fn is not None:
            ...
            # out_dict['truth_decode'] = truth_decode
            # out_dict['pred_decode'] = pred_decode
            ...
            pred_decode, truth_decode = pred, truth
    ...
    def forward(self, data_dict):
        ...
        truth = truth[indices].view(1, -1).to(x_in.device)

        # print(pred.shape, truth.shape)

        return pred.view(1, -1), truth    
