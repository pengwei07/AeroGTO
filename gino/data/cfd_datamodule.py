...
class VariableDictDataset(DictDataset):
    ...
    def load_wss(self, wss_path: Path) -> torch.Tensor:
        assert wss_path.exists(), "Info path does not exist"
        wss = np.load(str(wss_path))
        return torch.from_numpy(wss)
    ...
    def __getitem__(self, index):
        ...
        if self.path is not None:
            ...
            triangle_normals = torch.asarray(mesh.triangle_normals,dtype=torch.float64).reshape((-1,3))
            flow_normals = torch.zeros_like(triangle_normals)
            flow_normals[:,0]=-1

            mass_density = 1.0
            flow_speed = info['velocity']
            reference_area = info['width']*info['height']/2*1e-6 
            const = 2.0/(mass_density*(flow_speed**2)*reference_area)
            direction = torch.sum(triangle_normals*flow_normals, dim=1, keepdim=False)

            dragWeight = const*direction*areas
            ##########################
            ...
            # new
            return_dict["dragWeight"] = dragWeight
...
class VariableDictDatasetWithConstant(VariableDictDataset):
    def __init__(
        ...
        super().__init__(data_dict, path, localtion_norm, 
                         pressure_norm, info_norm, area_norm,wss_norm)
        self.constant_dict = constant_dict
...
class CFDDataModule(BaseDataModule):
    ...
    def encode(self, pressure: torch.Tensor) -> torch.Tensor:
        ...
        # self.output_normalization.to(pressure)
...
class AhmedBodyDataModule(CFDSDFDataModule):
    def __init__(
        ...
        min_area_bound, max_area_bound = [0.], [1.7468e-05]
        min_wss_bounds, max_wss_bounds = [-230,  -80,  -100],[30, 20 , 100 ]
        min_wss_bounds, max_wss_bounds =  np.array(min_wss_bounds), np.array(max_wss_bounds)
        ...
        wss_norm_fn = lambda x: self.wss_normalization(
            x, min_wss_bounds, max_wss_bounds
        )
